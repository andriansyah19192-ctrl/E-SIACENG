...
// ─── Kunci data yang disync ke Firebase (bukan siswa/guru — itu dari embedded) ───
const SYNC_KEYS = ['soal','ujian','ujian_online','jawaban_ujian','mapel','nilai',
                   'activity','fitur_baru','fitur_respons','sumber_belajar',
                   'perangkat_ajar','video_pembelajaran','naik_kelas','flayer_files'];
  ...
  try {
    ...
    _fbDb.ref('.info/connected').on('value', snap => {
      ...
      if (_fbOnline) {
        ...
        // Saat reconnect — push perubahan lokal yang mungkin tertinggal
        _pushPendingToFirebase();
      }
    });

    // ─── Real-time LISTENERS: terima perubahan dari Firebase → update state lokal ───
